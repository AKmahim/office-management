...
@section('title')
    <title>Edit Cash Out - Office Management</title>
@endsection
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Edit #{{ $cashOut->id }}</li>
                    </ol>
                </div>
                <h4 class="page-title">Edit Cash Out Record</h4>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-10 col-md-12 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Update Cash Out Details</h4>
                </div>
                <div class="card-body">
                    ...
                    <!-- Edit Form -->
                    <form action="{{ route('cashout.update', $cashOut->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" 
                                           ...
                                           id="source" 
                                           name="source" 
                                           value="{{ old('source', $cashOut->source) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" 
                                           ...
                                           id="reciever" 
                                           name="reciever" 
                                           value="{{ old('reciever', $cashOut->reciever) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" 
                                           ...
                                           id="given_by" 
                                           name="given_by" 
                                           value="{{ old('given_by', $cashOut->given_by) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control @error('payout_method') is-invalid @enderror" 
                                        ...
                                        <option value="cash" {{ old('payout_method', $cashOut->payout_method) == 'cash' ? 'selected' : '' }}>Cash</option>
                                        <option value="bank_transfer" {{ old('payout_method', $cashOut->payout_method) == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
                                        <option value="check" {{ old('payout_method', $cashOut->payout_method) == 'check' ? 'selected' : '' }}>Check</option>
                                        <option value="mobile_banking" {{ old('payout_method', $cashOut->payout_method) == 'mobile_banking' ? 'selected' : '' }}>Mobile Banking</option>
                                        <option value="online_payment" {{ old('payout_method', $cashOut->payout_method) == 'online_payment' ? 'selected' : '' }}>Online Payment</option>
                                        <option value="other" {{ old('payout_method', $cashOut->payout_method) == 'other' ? 'selected' : '' }}>Other</option>
                                        @foreach($payoutMethods as $method)
                                            @if(!in_array($method, ['cash', 'bank_transfer', 'check', 'mobile_banking', 'online_payment', 'other']))
                                                <option value="{{ $method }}" {{ old('payout_method', $cashOut->payout_method) == $method ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea class="form-control @error('note') is-invalid @enderror" 
                                      ...
                                      name="note" 
                                      rows="4" 
                                      placeholder="Additional notes, invoice number, reference details...">{{ old('note', $cashOut->note) }}</textarea>
                            @error('note')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            <div class="col-md-6">
                                <div class="card bg-light">
                                    <div class="card-body">
                                        ...
                                        <p class="mb-1"><strong>ID:</strong> #{{ $cashOut->id }}</p>
                                        <p class="mb-1"><strong>Created By:</strong> {{ $cashOut->createdBy->name ?? 'Unknown' }}</p>
                                        <p class="mb-1"><strong>Created:</strong> {{ $cashOut->created_at->format('M d, Y \a\t H:i') }}</p>
                                        <p class="mb-0"><strong>Updated:</strong> {{ $cashOut->updated_at->format('M d, Y \a\t H:i') }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-0">
                            <div class="text-right">
                                ...
                                <a href="{{ route('cashout.show', $cashOut->id) }}" class="btn btn-info mr-2">
                                    <i class="fe-eye"></i> View
                                </a>
                                <button type="submit" class="btn btn-danger">
                                    <i class="fe-save"></i> Update Cash Out
                                </button>
                            </div>
                        </div>
                    </form>
...
<script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        // Confirm update
        if (!confirm('Are you sure you want to update this cash out record?')) {
            e.preventDefault();
            return false;
        }
    });
