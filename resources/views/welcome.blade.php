...
<head>
    ...
    <style>
        ...
        .footer{
            ...
            min-height: 150px;
            width: 100%;
            background: linear-gradient(to bottom, #fef5e7, #620fc1);
            /* position: absolute; */
            bottom:0px;
            /* margin-top: 20rem; */
        }
    ...
    <div class="container">
        ...
        <form action="{{ route('find-content') }}" method="get" class="flex justify-center items-center flex-col mb-10">
            <input type="text" name="content_id"
             ...
             @if (session('error'))
                 <h1 class="text-center text-sm text-[#7c2fe8] mb-4">
                    {{ session('error') }}
                 </h1>
             @endif
            <button type="submit" 
            class="text-xl text-white font-medium px-6 py-2 rounded-lg bg-[#7c2fe8]"
            >Search</button>  <!-- Button to trigger the form submission -- !-->
        </form>

        <div class="footer bg-gradient-to-b from-[#fef5e7] to-[#620fc1] rounded-lg flex items-end justify-center h-20 relative">
